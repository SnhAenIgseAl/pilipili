const BiliMidCrc: any = function () {
    ...
    var crctable = new Array(256),
        ...
        crc32 = function (input: string | number) {
        ...
        crc32lastindex = function (input: string | number) {
            if (typeof (input) != 'string')
                input = input.toString();
            var crcstart = 0xFFFFFFFF, len = input.length, index;
            for (var i = 0; i < len; ++i) {
        ...
        deepCheck = function (i: string | number, index: any[]) {
            var tc = 0x00, str = '',
                hash = crc32(i);
